...
! numerous features from Fortran 66 and 77.  It is in fixed form format.  I have endeavored to modernize this
! code in two steps.  In this version I have left the code in fixed form and replaced the use of
! goto and line labels with conditional logic.  Additionally I have removed statement functions,
! made all variable declarations explicit, and added additional comments. 
! Additional routines were added as needed to reproduce the original behavior.  Original routines
! maintain their all caps format while new routines use camelcase.
!
! 	Whitespace in this document is inconsistent.  In particular the original code used partial
! indenting and idiosyncratic whitespace.  Original code and minimally modified code maintains this
! to make code comparison easier.  New and heavily modified code has been transitioned to style
! rules that favor full indenting and less internal whitespace.  Styling will be made consistent in
! the next modernization stage.
! 
! 	I have used tabs for code indenting and for alignment of comments.  This is to aid with
! modernization and porting of this code.  While tabs are not valid Fortran characters, compilers
! tend to tolerate them.  I have used 4-space equivalent tabs for layout purposes and to reduce line
! lengths but the original code uses ~3-4 space indenting.  Lines that use tabs to maintain lengths
! under 72 characters but look longer due to tab alignment have been marked with JMR_Length_OK_Tabs.
!
...
! Pg. 75: Start of source code.
     ...
     +				area, fint)
      	...
      	do
      		write(*, "(' Enter 1 to store fire history, 0 to skip ' ,$)") ! JMR_Length_OK_Tabs
      		read(*, *) ihist
...
! Pg. 76:
     ...
     +						ncalls, maxkl, tis, fint, fid )
      			...
      			call FIRINT(wodot, ash, htval, maxno, number, maxkl, area, ! JMR_Length_OK_Tabs
     +							fint, fi)
      			...
      			if (fi .LE. fimin) then
      				...
      				if (nohist .eqv. .false.) then
      					call STASH(tis, now, maxno, number, outfil, fi, flit, ! JMR_Length_OK_Tabs
     +								fout, wo, wodot, diam, ddot, tdry, tign, ! JMR_Length_OK_Tabs
     +								tout, fmois, maxkl, nun)
      				end if
      			end if
      		end do
     ...
     +					tis, ak, wdry, fmois, sigma, tign, tout, xmat, wo, diam) ! JMR_Length_OK_Tabs
      	close(nun)

      	do
      		write(*,
     +				"(' Exercise completed. Do another = 1 , quit = 0 ',$)") ! JMR_Length_OK_Tabs
...
!c Duff burning rate (ergo, intensity) and duration
      ...
      real, intent(in) :: dfm ! Ratio of moisture mass to dry organic mass / duff fractional moisture (aka R sub M)
...
! Pg. 79:
     ...
     +								ak, r0, dr, dt, ntimes, wdf, dfm)
      		...
      		do
      			write(*,
     +				"(' How many [ no more than'i3' ] fuel components ?    ',$)") ! JMR_Length_OK_Tabs
     +				maxno 
...
! Pg. 81:
     ...
     +											r0, dr, dt, ntimes, wdf, ! JMR_Length_OK_Tabs
     +											dfm)
      			!firstCall = .false.
      		end if
...
! Code extracted from GETDAT():
      ...
      subroutine GetComponentParameters(parts,
     +								wdry, ash, htval, fmois, dendry,
     +								sigma, cheat, condry, tpig, tchar, nFuel) ! JMR_Length_OK_Tabs
      	implicit none
      	...
      	write(*, "(//' Fuel component number'i3 ' properties list')")
     +																nFuel ! JMR_Length_OK_Tabs
      	write(*, "(' Name [ 12 characters or fewer ]    ', $ )")
      	read(*, "(a12)") name1
      	parts(nFuel) = name1

      	wdry(nFuel) = AskForReal1(name1, 'dry loading, kg / sq m ',
     +							small, big)

      	ash(nFuel) = AskForReal1(name1,
     +							'total mineral ash fraction    ',
     +							ash1, ash2)

      	htval(nFuel) = AskForReal1(name1,
     +							'heat of combustion, J / kg    ',
     +							htv1, htv2)

      	fmois(nFuel) = AskForReal1(name1,
     +							'moisture content, fraction    ',
     +							fms1, fms2)

      	dendry(nFuel) = AskForReal1(name1,
     +				'ovendry mass density, kg / cubic m    ',
     +				den1, den2)

      	sigma(nFuel) = AskForReal1(name1,
     +				'surface / volume ratio , inverse m    ',
     +				sig1, sig2)

      	cheat(nFuel) = AskForReal1(name1,
     +				'specific heat capacity ovendry , J / kg deg K  ',
     +				cht1, cht2)

      	condry(nFuel) = AskForReal1(name1,
     +				'ovendry thermal conductivity , W / m deg K    ',
     +				con1, con2)

      	tigi = AskForReal1(name1, 'ignition temperature, deg C    ',
     +						tig1, tig2)
      tpig(nFuel) = tigi + 273.0

      	tchi  = AskForReal1(name1,
     +				'char [end pyrolysis] temperature, deg C    ',
     +				tch1, tch2)
     ...
     +		'Enter igniting surface fire intensity , kW / sq m   ',
     +		fir1, fir2)
     
      	ti = AskForReal2(
     +		'Enter igniting surface fire residence time , s   ',
     +		ti1, ti2)

      	u = AskForReal2('Windspeed at top of fuelbed , m / s   ',
     +						u1, u2)

      	d = AskForReal2('Depth of fuelbed , m   ', d1, d2)

      	tamb = AskForReal2(' Ambient temperature , deg C   ',
     +						tam1, tam2)
      	tpamb = tamb + 273.0
      
      	do
      		write(*,
     +			"(' Dimensionless area influence factor [ak parameter]   ',$)")
      	...
      	do
      		write(*,
     +			"(' Fire environment minimum temperature parameter r0   ',$)")
      	...
      	do
      		write(*,
     +			"(' Fire environment increment temperature parameter dr  ',$)")
      	...
      	do
      		write(*,
     +			"(' Time step for integration of burning rates , s   ',$)")
      	...
      	wdf = AskForReal2('Duff dry weight loading, kg / sq m   ',
     +		wdf1, wdf2)
     
      	dfm = AskForReal2('Duff moisture fraction   ',
     +		dfm1, dfm2)
     ...
     +							sigma, cheat, condry, tpig, tchar, maxno, number, ! JMR_Length_OK_Tabs
     +							fi, ti, u, d, tpamb,
     +							ak, r0, dr, dt, wdf, dfm, ntimes)) then
      					exit ! Return to main program.
      				end if
      			else if (next .EQ. 1) then
      				call ReviewFuelComp(parts, wdry, ash, htval, fmois, dendry, ! JMR_Length_OK_Tabs
     +									sigma, cheat, condry, tpig, tchar, maxno, number) ! JMR_Length_OK_Tabs
     ...
     +							(n , parts(n) , wdry(n), n = 1 , number) ! Implied do loop.
      				...
      				write(*, "(' Index number of component to delete   ', $)") ! JMR_Length_OK_Tabs
     ...
     +							sigma, cheat, condry, tpig, tchar, maxno, number)
      	...
      	do
      		write(*, *)
     +				'Enter 0 to terminate review & revision of fuel data'
      		write(*, *)
     ...
     +								sigma, cheat, condry, tpig, tchar, maxno, number)
      ...
      subroutine ReviseFuelComponent(parts,
     +								wdry, ash, htval, fmois, dendry,
     +								sigma, cheat, condry, tpig, tchar, maxno, number)
      	implicit none
     ...
     +				(n, parts(n), wdry(n), n = 1, number) ! Implied do loop.
      		...
      		do
      			...
      			write(*,
     +				"(' Number component to be revised [ 0 = prev menu ]   ',$)")
      			...
      			if (readStat .ne. 0) then
      				...
      				if (nn .eq. 0) then
      					...
      					call ReviseFuelParameters(parts,
     +								wdry, ash, htval, fmois, dendry,
     +								sigma, cheat, condry, tpig, tchar, nn)
      ...
      ! History: This routine reproduces code originally in GETDAT() (block starting at label 50).
      ! Testing status: Reproduces original behavior.
      subroutine ReviseFuelParameters(parts,
     +								wdry, ash, htval, fmois, dendry,
     +								sigma, cheat, condry, tpig, tchar, nFuel)! number)
      	implicit none
      	...
      	do
      		...
      		write(*, "('   10'7x'ig temp C',e12.3)")
     +									(tpig(nFuel) - 273.0)
      		write(*, "('   11'7x'char temp',e12.3)")
     +									(tchar(nFuel) - 273.0)
      		write(*,
     +		"('   enter number param to change [ 0 = prev menu ]   ',$)")
      ...
      subroutine ReviewFireEnvData(fi, ti, u, d, tpamb)
      	...
      	do
      		...
      		write(*,
     +		"('  1 - Igniting fire intensity, kW/ sq m      'e12.3)") fi
      		write(*,
     +		"('  2 - Igniting fire residence time, s        'e12.3)") ti
      		write(*,
     +		"('  3 - windspeed at top of fuel bed, m / s    'e12.3)") u
      		write(*,
     +		"('  4 - fuelbed depth, m                       'e12.3)") d
      		write(*,
     +		"('  5 - ambient temperature, deg C             'e12.3)")
     +														(tpamb - 273.0)

      		do
      			write(*,
     +			"(' Index of parameter to change [ 0 = prev menu ]   ',$)")
      ...
      subroutine ReviewIntlCntlVars(ak, r0, dr, dt, wdf, dfm, ntimes)
      	...
      	do
      		! Parameter selection menu:
      		write(*, 
     +			"('  1 - Area influence factor [ ak parameter ]       'e12.3)")
     +			ak
      		write(*,
     +			"('  2 - Fire environment temperature parameter r0    'e12.3)")
     +			r0
      		write(*,
     +			"('  3 - Fire environment temperature parameter dr    'e12.3)")
     +			dr
      		write(*,
     +			"('  4 - Time step to integrate burning rates , s     'e12.3)")
     +			dt
      		write(*,
     +			"('  5 - Duff dry weight loading , kg / sq m          'e12.3)")
     +			wdf
      		write(*,
     +			"('  6 - Duff moisture content , fraction dry weight  'e12.3)")
     +			dfm
      		write(*,
     +			"('  7 - Number time steps                            'i4)")
     +			ntimes
      		write(*,
     +		"('  Index of parameter to change [ 0 = prev menu ]   ',$)")
     ...
     +								ak, r0, dr, dt, ntimes, wdf, dfm)
      	...
      	character(len = *), parameter :: format2005 =
     +											"(2x,5e15.3/2x,5e15.3)"
      	...
      	do
      		write(*,
     +				"(' Enter name of file holding fuel component data   ',$)")
     ...
     +				iostat = openStat)
      		if (openStat .eq. 0) then
      			...
      			do
      				write(*,
     +					"(' Error opening input file  ', a12'  reenter file name')")
     +					infile
      				read(*, format08, iostat = readStat) infile
      				if (readStat .eq. 0) then
     ...
     +								condry(n), tpig(n), tchar(n)
      	...
      	! This is so ugly!
      	write(*,
     +		"(' Enter name of file holding igniting fire, environmental'/"//
     ...
     +				iostat = openStat)
      		if (openStat .eq. 0) then
      			...
      			do
      				write(*,
     +					"(' Error opening input file  ', a12'  reenter file name')") ! JMR_Length_OK_Tabs
     +						infile
      				read(*, format08, iostat = readStat) infile
      				if (readStat .eq. 0) then
      					exit ! Exit to out loop and try to open the new file.
     ...
     +							ak, r0, dr, dt, wdf, dfm, ntimes) result(exitMenus)
      	...
      	!character*12, intent(out) :: outfil ! ?????
     ...
     +									"(2x,5e15.3/2x,5e15.3)"
      	...
      	do
      		write(*, *)
     +				'Enter file name [ 12 char or fewer ] for storage of the'
      		write(*, "(' fuel component data currently in use   ',$)")
      		
      		read(*, format08, iostat = readStat) outfil
      		if (readStat .eq. 0) then
      			open(fun, file = outfil, status = 'UNKNOWN',
     +							form = 'FORMATTED', iostat = openStat)
      			if (openStat .ne. 0) then
      				write(*,
     +					"(' Error opening output file  'a12'  enter another name')")
     +					outfil
      			else
      	...
      	write(fun,
     +			"(i6,' Fuel component properties -- 3 records per entry')")
     +			number
     ...
     +				'Enter file name [ 12 char or fewer ] for storage of the'
      		write(*,
     +		"(' igniting fire, environmental, and control data used  ',$)")
...
! Pg. 95:
      ...
      ! Local constants:
      real, parameter :: rindef = 1.e+30 ! JMR_NOTE: Make this global?
     ...
     +                  tdry , tign , tout , fmois , maxkl , nun)
      ...
      if( now .EQ. 1) then
      	...
      	do ! Loop until valid input is received. Alt: Set istash = -1, do while (istash .ne. 0 .and. istash .ne. 1)
      		write(*,
     +				"(' Enter 1 to stash fuelbed snapshots, O to skip ', $)") ! JMR_Length_OK_Tabs
      		read( * , * ) istash
      		if (istash .EQ. 0) then
      			exit
      	...
      	if (istash .EQ. 1) then
      		...
      		do ! Name loop: Loop until a valid file is specified.
      			write(*,
     +					"(' File name [ max 12 char ] for STASH output    ', $)") ! JMR_Length_OK_Tabs
      			read(*, fmt = '(a12)') outfil

      			open(nun, file = outfil, status = 'NEW', form = 'FORMATTED', ! JMR_Length_OK_Tabs
     +					iostat = openStat)
...
! Pg. 101:
      				...
      				write(*,  "(' Enter 2 to terminate program now    ', $)") ! JMR_Length_OK_Tabs
      				...
      				if (ido .EQ. 0) then
      					...
      					open(nun, file = outfil, status = 'NEW',
     +							form = 'FORMATTED', iostat = openStat)
...
!c dimensionless mixing parameter r.
      ...
      do
      	den = 1. + term * ( rlast + 1. ) * ( rlast * rlast + 1. )
...
! Pg. 112:
      ...
      ! This writes to file a summary of fuel consumption by size class, in one of two levels of
      ! detail.
      ! History: Modernized original Burnup subroutine.
     ...
     +tis , ak , wdry , fmois , sigma , tign , tout , xmat , wo , diam )
      ...
      integer :: openStat ! JMR_MOD: IO status.
     ...
     +"(/5x,a12,3x'Load , mois , diam =  '3e10.3/5x
     +'  Companion  fraction   Ignition   burnout   load rem    diam')"
      character(len = *), parameter :: format30 =
     +							'(5x,a12,e9.3,e11.3,e10.3,e11.3,e9.3)'
      character(len = *), parameter :: format40 =
     +							'(5x,a12,e9.3,e11.3,e10.3,e11.3)'
      ...
      open(nun, file = nuname, status = stat, form = 'FORMATTED',
     +		iostat = openStat)
      
      ! If there is an error opening the file prompt the user until resolved:
      do while (openStat .ne. 0)
      	write(*, "('  Error on open:  'a12'  
     +with status = 'a3)") nuname, stat ! JMR: Ugly wrapping.
      	write(*, *) ' 0 = abort now'
     ...
     +			iostat = openStat)
      ...
      write(nun, 
     +"(5x'Area factor =' , f6.2 , '  End time , s =' , e12.4 )") 
     +ak , tis

      if (.NOT. v) then ! ido must == 0, recap only
      	write(nun,
     +			"(6x'Component   load @ 0   Ignition   burnout   load rem')") ! JMR_Length_OK_Tabs
      end if
...
! Pg. 113:
      ...
      !return ! JMR_NOTE: Rendered superflous by modifications.
...
! JMR_MOD_START:
      ...
      !
      ! History: This function was originally implemented as a statement function in defined in
      ! seven places in the original code.
      function Loc(k, l)! result(kl)
      	 implicit none
      	...
      	! Locals:
      	integer :: loc !kl ! Return value: Index in a compact array representing the triangular matrix values.
      ...
      ! To reproduce the original behavior leading spaces should be omitted.
      function AskForReal1(cName, paramDesc, rangeLow, rangeHigh)
     +													result(input)
      	implicit none
      	...
      	character(len = len(fmtStart) + len(paramDesc) +
     +									len(fmtEnd)) :: formatString
      	integer :: readStat ! IO error status.
      	logical :: startAgain
     ...
     +									len(fmtEnd)) :: formatString
      ...
      function AskForReal(paramDesc, rangeLow, rangeHigh, cName)
     +														result(input)
      	implicit none
      	...
      	real :: input ! Return value.
      	character(len = len(fmtStart) + len(paramDesc) +
     +									len(fmtEnd)) :: formatString ! Longest case!
      	integer :: readStat ! IO error status.
      	logical :: startAgain
